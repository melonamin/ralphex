...
func TestBuffer_Add(t *testing.T) {
	t.Run("adds events", func(t *testing.T) {
		...
		e1 := NewOutputEvent(processor.PhaseTask, "first")
		e2 := NewOutputEvent(processor.PhaseTask, "second")
	...
	t.Run("overwrites oldest when full", func(t *testing.T) {
		...
		b.Add(NewOutputEvent(processor.PhaseTask, "first"))
		b.Add(NewOutputEvent(processor.PhaseTask, "second"))
		b.Add(NewOutputEvent(processor.PhaseTask, "third"))
		b.Add(NewOutputEvent(processor.PhaseTask, "fourth"))
...
func TestBuffer_All(t *testing.T) {
	...
	t.Run("returns all events in order", func(t *testing.T) {
		...
		b.Add(NewOutputEvent(processor.PhaseReview, "second"))
		time.Sleep(time.Millisecond)
		b.Add(NewOutputEvent(processor.PhaseCodex, "third"))
	...
	t.Run("returns events in order after wrap", func(t *testing.T) {
		...
		for i := range 5 {
			time.Sleep(time.Millisecond)
			b.Add(NewOutputEvent(processor.PhaseTask, string(rune('A'+i))))
		}
...
func TestBuffer_ByPhase(t *testing.T) {
	t.Run("returns nil for empty phase", func(t *testing.T) {
		b := NewBuffer(10)
		assert.Nil(t, b.ByPhase(processor.PhaseTask))
	})

	t.Run("filters by phase", func(t *testing.T) {
		b := NewBuffer(10)

		b.Add(NewOutputEvent(processor.PhaseTask, "task1"))
		time.Sleep(time.Millisecond)
		b.Add(NewOutputEvent(processor.PhaseReview, "review1"))
		time.Sleep(time.Millisecond)
		b.Add(NewOutputEvent(processor.PhaseTask, "task2"))
		time.Sleep(time.Millisecond)
		b.Add(NewOutputEvent(processor.PhaseCodex, "codex1"))

		taskEvents := b.ByPhase(processor.PhaseTask)
		require.Len(t, taskEvents, 2)
		assert.Equal(t, "task1", taskEvents[0].Text)
		assert.Equal(t, "task2", taskEvents[1].Text)

		reviewEvents := b.ByPhase(processor.PhaseReview)
	...
	t.Run("handles phase after wrap", func(t *testing.T) {
		...
		b.Add(NewOutputEvent(processor.PhaseTask, "task2"))
		time.Sleep(time.Millisecond)
		b.Add(NewOutputEvent(processor.PhaseReview, "review2"))
		time.Sleep(time.Millisecond)
		// this overwrites task1
		b.Add(NewOutputEvent(processor.PhaseTask, "task3"))
...
func TestBuffer_Concurrency(t *testing.T) {
	...
	for range 5 {
		wg.Go(func() {
			for range 50 {
				_ = b.All()
				_ = b.ByPhase(processor.PhaseTask)
				_ = b.Count()
			}
		})
	}
